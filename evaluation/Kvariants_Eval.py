...
import pandas as pd
from Additional_Code_BULE.loe_utils.helperfunctions import save_allresults_pickle
class KVariantEval:

    def __init__(self, dataset, exp_path, model_configs,contamination,query_num,assumed_contamination,trainset_fraction):# BULE assumed_contamination
        ...
        # BULE
        self._PICKLE_FOLDER ='PICKLE'
        self._assumed_contamination = assumed_contamination
        self._trainset_fraction = trainset_fraction
    ...
    def risk_assessment(self, experiment_class):
        ...
        #BULE
        self._PICKLE_PATH = os.path.join(self._NESTED_FOLDER, self._PICKLE_FOLDER)
        if not os.path.exists(self._PICKLE_PATH):
                os.makedirs(self._PICKLE_PATH)
        ...
        # if done then call the merge picklefiles 
        save_allresults_pickle(self._PICKLE_PATH)
    ...
    def _risk_assessment_helper(self, cls, experiment_class, exp_path):
        ...
        #BULE this function gets called for every class by risk assessement
        ...
        for i in range(num_repeat):
            ...
            if self._trainset_fraction == None or self._trainset_fraction ==  0.0: 
                trainset, valset, testset = load_data(self.data_name, cls, self.contamination,trainset_name='trainset_2048.pt')
                print('trainset_2048.pt')
            
            else:
                trainset, valset, testset = load_data(self.data_name, cls, self.contamination,trainset_name=f'trainset_2048_fraction_{self._trainset_fraction}.pt')
                print('trainset_2048_fraction_{self._trainset_fraction}.pt')


            ### BULE -> WHY IS SELF CONTAMINATION PASSED TO RUN TEST??
            #val_auc, test_auc, test_ap,test_f1, test_score,target_all = experiment.run_test(trainset,valset,testset,logger,self.contamination,self.query_num)

            #BULE -> pass assumed contamination to run test
            val_auc, test_auc, test_ap,test_f1, test_score,target_all = experiment.run_test(trainset,valset,testset,logger,self._assumed_contamination,self.query_num)
            
            #BULE need target and score vectors here -> for each class -> 
            #save each class and run as pickle file -> 50 pickle files- > then run script tp gather all the classes  
            
            contamination_ratio =  np.full_like(test_score, self.contamination)
            normal_label =         np.full_like(test_score, cls )
            
             
            # create dataframe from arrays
            df_ =pd.DataFrame({'score_':test_score,'anomaly_':target_all,'normal_label':normal_label,'contam_ratio':contamination_ratio})#, 
            
            SAVE_PATH=os.path.join(self._PICKLE_PATH, f'result_cls_{cls}_run_{i}.pkl')
            #save as pkl
            df_.to_pickle(SAVE_PATH)
