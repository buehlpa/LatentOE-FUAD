...
def FMNIST_feat(normal_class, root='DATA/fmnist_features/',contamination_rate=0.0,trainset_name='trainset_2048.pt'):
    #loading feature datasets
    trainset = torch.load(root + trainset_name)
    train_data, train_targets = trainset
    testset = torch.load(root + 'testset_2048.pt')
    test_data, test_targets = testset
...
def load_data(data_name,cls,contamination_rate=0.0,trainset_name='trainset_2048.pt'):
    ...
    if data_name == 'cifar10_feat':
        ...
        train, train_label, test, test_label = FMNIST_feat(cls, contamination_rate=contamination_rate,trainset_name=trainset_name)
