...
class NeutralAD_trainer:
    ...
    def __init__(self, model, loss_function,config,**kwargs):
    ...
    def _train(self, epoch,train_loader, optimizer):
        ...
        for data in train_loader:
            samples = data['sample']
            labels = data['label']
            samples = samples.to(self.device) # was commented
            ...
            if epoch <=self.warmup:
                ...
                elif self.train_method=='simple_refine':
                    loss = loss_n
                    loss_mean = loss.mean()
    ...
    def detect_outliers(self, loader):
        ...
        target_all = np.concatenate(target_all) #BULE need these vectors for each class 
        
       
        # if flag and method is simple refine only return the score reswt no needed
        if self.train_method=='simple_refine' and 'simple_refine_flag' in self.kwargs and self.kwargs['simple_refine_flag']:
            return score_all
        ...
        return auc, ap,f1,  score_all,loss_in.item() / (target_all == 0).sum(), loss_out.item() / (target_all == 1).sum(),target_all


    def train(self, train_loader, contamination, query_num=0,optimizer=None, scheduler=None,
              validation_loader=None, test_loader=None, early_stopping=None, logger=None, log_every=2,**kwargs):
        ...
        for epoch in range(1, self.max_epochs+1):
            ## TODO Put the refined data here after cutting n% of the data and continue training blind 
            ...
            if test_loader is not None:
                test_auc, test_ap,test_f1, test_score, testin_loss,testout_loss,target_all = self.detect_outliers(test_loader)

            if validation_loader is not None:
                val_auc, val_ap,val_f1, _, valin_loss,valout_loss,_ = self.detect_outliers(validation_loader)
                if epoch>self.warmup:
        ...
        #BULE need to return the score/ target vectors
        return val_loss, val_auc, test_auc, test_ap,test_f1, test_score,target_all